...
__declspec(noinline) DunGen::CVoxelCave::CVoxelCave(const CRandomGenerator* randomGenerator_)
	...
	, Border(SVoxelSpace::MinBorder+1)
	, MinDrawRadius(2)
...
unsigned int DunGen::CVoxelCave::GetBorder() const
{
	return Border;
}
...
unsigned int DunGen::CVoxelCave::GetMinDrawRadius() const
{
	return MinDrawRadius;
}
...
inline void DunGen::CVoxelCave::DrawVoxelBlock(unsigned int x_,unsigned int y_,unsigned int z_)
{
	...
	Voxel[x_-1][y_][z_] = 1; Voxel[x_][y_-1][z_] = 1; Voxel[x_][y_][z_-1] = 1;	
	Voxel[x_+1][y_][z_] = 1; Voxel[x_][y_+1][z_] = 1; Voxel[x_][y_][z_+1] = 1;

	// draw 18-connected voxels
	Voxel[x_-1][y_-1][z_] = 1; Voxel[x_-1][y_+1][z_] = 1; Voxel[x_+1][y_-1][z_] = 1; Voxel[x_+1][y_+1][z_] = 1;
	Voxel[x_-1][y_][z_-1] = 1; Voxel[x_-1][y_][z_+1] = 1; Voxel[x_+1][y_][z_-1] = 1; Voxel[x_+1][y_][z_+1] = 1;
	Voxel[x_][y_-1][z_-1] = 1; Voxel[x_][y_-1][z_+1] = 1; Voxel[x_][y_+1][z_-1] = 1; Voxel[x_][y_+1][z_+1] = 1;	
}
	...
	for (unsigned int i=SVoxelSpace::MinBorder; i<=xMax; ++i)
		for (unsigned int j=SVoxelSpace::MinBorder; j<=yMax; ++j)
			for (unsigned int k=SVoxelSpace::MinBorder; k<=zMax; ++k)
			{
				...
				if (1 == Voxel[i-1][j][k] + Voxel[i][j][k]) {++triangleCounter;}
				if (1 == Voxel[i][j-1][k] + Voxel[i][j][k]) {++triangleCounter;}
				if (1 == Voxel[i][j][k-1] + Voxel[i][j][k]) {++triangleCounter;}
			}
	...
	double angleYaw_, double anglePitch_, double angleRoll_, double radiusStart_, double radiusFactor_, double radiusDecrement_)
{
	...
	const double angleYawRAD = angleYaw_ * irr::core::DEGTORAD64;
	const double angleNickRAD = anglePitch_ * irr::core::DEGTORAD64;
	const double angleRollRAD = angleRoll_ * irr::core::DEGTORAD64;
...
inline bool DunGen::CVoxelCave::IsBoundaryVoxel(unsigned int x_,unsigned int y_,unsigned int z_) const
{
	...
	if (1 == Voxel[x_-1][y_][z_] || 1 == Voxel[x_+1][y_][z_] ||
		1 == Voxel[x_][y_-1][z_] || 1 == Voxel[x_][y_+1][z_] ||
		1 == Voxel[x_][y_][z_-1] || 1 == Voxel[x_][y_][z_+1])
		return true;
...
inline bool DunGen::CVoxelCave::IsBoundaryVoxelExtended(unsigned int x_,unsigned int y_,unsigned int z_) const
{
	...
	if (1 == Voxel[x_-1][y_-1][z_] || 1 == Voxel[x_-1][y_+1][z_] ||
		1 == Voxel[x_+1][y_-1][z_] || 1 == Voxel[x_+1][y_+1][z_] ||

		1 == Voxel[x_-1][y_][z_-1] || 1 == Voxel[x_-1][y_][z_+1] ||
		1 == Voxel[x_+1][y_][z_-1] || 1 == Voxel[x_+1][y_][z_+1] ||
		
		1 == Voxel[x_][y_-1][z_-1] || 1 == Voxel[x_][y_-1][z_+1] ||
		1 == Voxel[x_][y_+1][z_-1] || 1 == Voxel[x_][y_+1][z_+1])
		return true;
...
unsigned int DunGen::CVoxelCave::Filter()
{
	...
	while (!queueVoxel.empty())
	{
		...
		if (IsBoundaryVoxelExtended(actVoxel.X-1,actVoxel.Y,actVoxel.Z))
		{
			...
			// then mark voxel
			Voxel[newVoxel.X][newVoxel.Y][newVoxel.Z] = HelperVoxel;
		}
		if (IsBoundaryVoxelExtended(actVoxel.X+1,actVoxel.Y,actVoxel.Z))
		{
			// create new voxel
	...
	for (unsigned int i=lastXofSearch; i<SVoxelSpace::DimX-MinBorderFilter; ++i)
		for (unsigned int j=MinBorderFilter; j<SVoxelSpace::DimY-MinBorderFilter; ++j)
			for (unsigned int k=MinBorderFilter; k<SVoxelSpace::DimZ-MinBorderFilter; ++k)
				if (HelperVoxel == Voxel[i][j][k])
					Voxel[i][j][k] = 0;		// original marking was 0
