			...
			for (unsigned int j=actualOctreeNode->BorderMinX; j<=actualOctreeNode->BorderMaxX; ++j)
			{
				...
				for (unsigned int k=actualOctreeNode->BorderMinY; k<=actualOctreeNode->BorderMaxY; ++k)
					for (unsigned int l=actualOctreeNode->BorderMinZ; l<=actualOctreeNode->BorderMaxZ; ++l)
					{
						...
						if (1 == VoxelCave->Voxel[j][k][l])
						{
							if (0 == VoxelCave->Voxel[j-1][k][l])
							{
			...
			for (unsigned int i=actualOctreeNode->BorderMinX; i<=actualOctreeNode->BorderMaxX; ++i)
			{
				...
				for (unsigned int j=actualOctreeNode->BorderMinY; j<=actualOctreeNode->BorderMaxY; ++j)
					for (unsigned int k=actualOctreeNode->BorderMinZ; k<=actualOctreeNode->BorderMaxZ; ++k)
					{
						...
						if (1 == VoxelCave->Voxel[i][j][k])
						{		
							// test along X-axis
							if (0 == VoxelCave->Voxel[i-1][j][k])
							{
...
irr::core::vector3d<irr::f32> DunGen::CMeshCave::ComputeVertexCoordinates(unsigned int x_, unsigned int y_, unsigned int z_, irr::f32& markingDockingVertex_)
{
	...
	bool dockingVertex = (CVoxelCave::DockingVoxel == (VoxelCave->Voxel[x_-1][y_][z_]|VoxelCave->Voxel[x_][y_-1][z_]
		|VoxelCave->Voxel[x_][y_][z_-1]|VoxelCave->Voxel[x_-1][y_-1][z_]|VoxelCave->Voxel[x_-1][y_][z_-1]
		|VoxelCave->Voxel[x_][y_-1][z_-1]|VoxelCave->Voxel[x_-1][y_-1][z_-1]|VoxelCave->Voxel[x_][y_][z_]) );
	...
	if (SmoothEnabled && !dockingVertex)
	{
		...
		unsigned int indexWarpTable = (VoxelCave->Voxel[x_-1][y_-1][z_-1]) | ((VoxelCave->Voxel[x_][y_-1][z_-1])<<1)
			| ((VoxelCave->Voxel[x_-1][y_][z_-1])<<2) | ((VoxelCave->Voxel[x_][y_][z_-1])<<3)
			| ((VoxelCave->Voxel[x_-1][y_-1][z_])<<4) | ((VoxelCave->Voxel[x_][y_-1][z_])<<5)
			| ((VoxelCave->Voxel[x_-1][y_][z_])<<6) | ((VoxelCave->Voxel[x_][y_][z_])<<7);
...
irr::f32 DunGen::CMeshCave::IsBorderVertex(unsigned int x_, unsigned int y_, unsigned int z_, SOctreeNode* octreeNode_)
{
	...
	if (x_ == octreeNode_->BorderMinX)
	{
		...
		if (1 == VoxelCave->Voxel[x_-1][y_][z_])
			if (0 == (VoxelCave->Voxel[x_][y_][z_] * VoxelCave->Voxel[x_-1][y_-1][z_]
				* VoxelCave->Voxel[x_-1][y_][z_-1]))
				return 1.0f;

		if (1 == VoxelCave->Voxel[x_-1][y_-1][z_])
			if (0 == (VoxelCave->Voxel[x_][y_-1][z_] * VoxelCave->Voxel[x_-1][y_][z_]
				* VoxelCave->Voxel[x_-1][y_-1][z_-1]))				
				return 1.0f;

		if (1 == VoxelCave->Voxel[x_-1][y_][z_-1])
			if (0 == (VoxelCave->Voxel[x_][y_][z_-1] * VoxelCave->Voxel[x_-1][y_-1][z_-1]
				* VoxelCave->Voxel[x_-1][y_][z_]))
				return 1.0f;

		if (1 == VoxelCave->Voxel[x_-1][y_-1][z_-1])
			if (0 == (VoxelCave->Voxel[x_][y_-1][z_-1] * VoxelCave->Voxel[x_-1][y_][z_-1]
				* VoxelCave->Voxel[x_-1][y_-1][z_]))
				return 1.0f;
	}
	// test the 4 voxels in the adjacent region
	...
	{
		// is one border voxel of the adjacent region 1 and an arbitrary adjacent border voxel 0?
		if (1 == VoxelCave->Voxel[x_][y_][z_])
			if (0 == (VoxelCave->Voxel[x_-1][y_][z_] * VoxelCave->Voxel[x_][y_-1][z_]
				* VoxelCave->Voxel[x_][y_][z_-1]))
				return 1.0f;

		if (1 == VoxelCave->Voxel[x_][y_-1][z_])
			if (0 == (VoxelCave->Voxel[x_-1][y_-1][z_] * VoxelCave->Voxel[x_][y_][z_]
				* VoxelCave->Voxel[x_][y_-1][z_-1]))
				return 1.0f;

		if (1 == VoxelCave->Voxel[x_][y_][z_-1])
			if (0 == (VoxelCave->Voxel[x_-1][y_][z_-1] * VoxelCave->Voxel[x_][y_-1][z_-1]
				* VoxelCave->Voxel[x_][y_][z_]))
				return 1.0f;

		if (1 == VoxelCave->Voxel[x_][y_-1][z_-1])
			if (0 == (VoxelCave->Voxel[x_-1][y_-1][z_-1] * VoxelCave->Voxel[x_][y_][z_-1]
				* VoxelCave->Voxel[x_][y_-1][z_]))
				return 1.0f;
	}
	// test the 4 voxels in the adjacent region
	if (y_ == octreeNode_->BorderMinY)
	{
		...
		if (1 == VoxelCave->Voxel[x_-1][y_-1][z_])
			if (!(VoxelCave->Voxel[x_][y_-1][z_] * VoxelCave->Voxel[x_-1][y_][z_]
				* VoxelCave->Voxel[x_-1][y_-1][z_-1]))
				return 1.0f;
