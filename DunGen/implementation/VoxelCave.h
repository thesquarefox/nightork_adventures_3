...
namespace DunGen
{
	...
	// the mesh cave is our friend, because it can convert us to triangles
	friend class CMeshCave;
	...
	public:
		...
		/// get the minimum voxel space border (this is always 0 = stone, cannot be smaller than 3)
		unsigned int GetBorder() const;

		/// set the minimal draw radius for turtle graphic
		void SetMinDrawRadius(unsigned int minDrawRadius_);

		/// get the minimal draw radius for turtle graphic
		unsigned int GetMinDrawRadius() const;
