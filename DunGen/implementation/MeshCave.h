...
namespace DunGen
{
	...
	public:
		...
		/// get parameter warp random seed
		unsigned int GetWarpRandomSeed() const;

		/// set parameter warp strength, returns resulting warp strength (clamped to [0,0.49])
		void SetWarpStrength(double value_);

		/// get parameter warp strength, returns resulting warp strength (clamped to [0,0.49])
		double GetWarpStrength() const;

		/// specifies if warping should be used
		void SetWarpOption(bool enabled_);

		/// shows if warping should be used
		bool GetWarpOption() const;

		/// specifies if warping should be smoothing the mesh
		void SetSmoothOption(bool enabled_);

		/// shows if warping should be smoothing the mesh
		bool GetSmoothOption() const;
		
		/// sets the method for normal weighting
		void SetNormalWeightMethod(ENormalWeightMethod::Enum normalWeightMethod_);

		/// gets the method for normal weighting
		ENormalWeightMethod::Enum GetNormalWeightMethod() const;
