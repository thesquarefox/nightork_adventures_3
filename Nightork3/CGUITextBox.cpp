...
namespace gui
{
CGUITextBox::CGUITextBox(irr::gui::IGUIFont * font, const irr::video::SColor& color, bool centered,
	const wchar_t * text,  IGUIEnvironment* environment, core::rect<s32> rectangle,
...
void CGUITextBox::reset(void)
{
	...
	m_pStaticText->setOverrideColor(m_pColor);
	m_pStaticText->enableOverrideColor(true);
	m_pStaticText->setBackgroundColor(irr::video::SColor(255,0,0,0));
	if (m_centered)
	{
		m_pStaticText->setTextAlignment(EGUIA_CENTER,EGUIA_UPPERLEFT);
	}
